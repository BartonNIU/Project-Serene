...
$(function() {
           //$("button").click(function () {
               $.get("php/mysql.php", function (data) {
                   var locations = JSON.parse(data);
                   //$(".listsearch-header").html(data);
                   //var lol = eval("("+locations[1]["coordinate"]+")");
                   //var lol = JSON.parse(locations[1]["coordinate"]);
                   //var lol = new String(locations[1]["coordinate"]);
                   //var lol =
                   //$(".listsearch-header").html(typeof(lol));
                    //var latitude = parseFloat("-37.8142");
                    //var longitude = parseFloat("144.96");
                   //$(".listsearch-header").html({lat: latitude, lng: longitude});
                   $(".listsearch-header").html(locations[1]["location"]);
//(function($){
                   "use strict";
                   var markerIcon = {
                   ...
                   function mainMap() {
                       ...
                       var map = new google.maps.Map(document.getElementById('map-main'), {
                           zoom: 12,
                           scrollwheel: false,
                           center: eval("("+locations[1]["coordinate"]+")"),//new String(locations[1]["coordinate"]), //{lat:-37.8142, lng:144.9632},{lat: latitude, lng: longitude},
                       ...
                       for (i = 0; i < locations.length; i++) {
                           marker = new google.maps.Marker({
                               position: eval("("+locations[i]["coordinate"]+")"),
                               //icon: locations[i][4],
                               id: i
                           });

                           // the code below is for show the popup on map
                           //locationData("",locations[i]["categoty"],locations[i]["location"],locations[i]["coordinate"],"0400600");
                           ...
                           google.maps.event.addListener(marker, 'click', (function (marker, i) {
                               return function () {
                                   ...
                                   boxText.innerHTML = locationData("",locations[i]["categoty"],locations[i]["location"],"","0400600");
                                   ib.close();
                                   ib.open(map, marker);
                                   currentInfobox = marker.id;
                                   var latLng = eval("("+locations[i]["coordinate"]+")");
                                   map.panTo(latLng);
                                   map.panBy(0, -180);
                                   google.maps.event.addListener(ib, 'domready', function () {
